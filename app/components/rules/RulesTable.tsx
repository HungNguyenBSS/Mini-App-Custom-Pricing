...
export function RulesTable({ rules, loading, selectedResources, allResourcesSelected, onSelectionChange, mode, setMode, queryValue, selectedTab, sortSelected, onQueryChange, onQueryClear, onTabChange, onSort, onNavigate, onDuplicate, onDelete, onBulkAction }: Props) {
  return (
    <>
      ...
      <IndexTable resourceName={{ singular: "rule", plural: "rules" }} itemCount={rules.length} selectedItemsCount={allResourcesSelected ? "All" : selectedResources.length} onSelectionChange={onSelectionChange} loading={loading} bulkActions={[{ content: "Enable", onAction: () => onBulkAction("enable") }, { content: "Disable", onAction: () => onBulkAction("disable") }, { content: "Delete", onAction: () => onBulkAction("remove") }]} headings={[{ title: "Name" }, { title: "Status" }, { title: "Priority" }, { title: "Created Date" }, { title: "Updated Date" }, { title: "Action" }]}>
